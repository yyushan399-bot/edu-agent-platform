...
class TheoryAgentOutput(BaseModel):
    ...
    concept_accuracy_score: int = Field(
        ge=1, le=5, description="概念准确性 1-5 分"
    )
    logic_integrity_score: int = Field(
        ge=1, le=5, description="逻辑完整性 1-5 分"
    )
    theory_transfer_score: int = Field(
        ge=1, le=5, description="理论迁移 1-5 分"
    )
...
THEORY_RUBRIC_BLOCK = format_dimension_rubric_block("theory")
...
THEORY_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "4. concept_accuracy_score / logic_integrity_score / theory_transfer_score："
            "各 1-5 整数，须严格对照下方量规\n"
...
def _to_theory_result(output: TheoryAgentOutput) -> TheoryResult:
    ...
    subs = [
        clamp_sub_score(output.concept_accuracy_score),
        clamp_sub_score(output.logic_integrity_score),
        clamp_sub_score(output.theory_transfer_score),
    ]
    return {
        ...
        "concept_accuracy_score": subs[0],
        "logic_integrity_score": subs[1],
        "theory_transfer_score": subs[2],

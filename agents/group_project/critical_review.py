...
你只审核 critical_agent 已经给出的【{dimension_name}】维度评分结果，不重新评分，不替学生补充内容。
...
class IndependentAuditAgent:
    ...
    def _print_check_result(self, state: Dict[str, Any], *, check_name: str, passed: bool, issues: Optional[List[str]] = None, warnings: Optional[List[str]] = None) -> None:
        ...
        self._verbose_print(state, f"[critical_Review]   {check_name}：{result_text}")
        for issue in issues or []:
            if issue.strip():
                self._verbose_print(state, f"[critical_Review]     问题：{issue}")
        for warning in warnings or []:
            if warning.strip():
                self._verbose_print(state, f"[critical_Review]     提醒：{warning}")
    ...
    def audit_one_dimension(self, *, dimension_key: str, dimension_result: Dict[str, Any], rubric: str, merged_report_context: str, state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        self._verbose_print(state, f"[critical_Review] 开始审核维度：{dimension_name} ({dimension_key})")

        self._verbose_print(state, "[critical_Review]   正在审核：多次评分稳定性")
        ...
        self._verbose_print(state, "[critical_Review]   正在审核：学生报告真实性 / student_dimension_text 可追溯性")
        ...
        self._verbose_print(state, "[critical_Review]   正在审核：量规一致性、分数—理由一致性")
        ...
        if audit_comment:
            self._verbose_print(state, f"[critical_Review]   语义审核总结：{audit_comment}")
        ...
        self._verbose_print(state, f"[critical_Review] 维度审核结果：{dimension_name} {'通过' if passed else '未通过'}")
        if failed_checks:
            self._verbose_print(state, f"[critical_Review]   未通过项目：{failed_checks}")
    ...
    def audit_all_dimensions(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._verbose_print(state, "[critical_Review] ============================================================")
        if audit_only_failed_dimensions:
            self._verbose_print(state, f"[critical_Review] 本轮只重新审核上一轮未通过并已重评的维度：{all_keys}")
            self._verbose_print(state, f"[critical_Review] 待审核维度数量：{len(all_keys)}（仅审核本轮重评维度）")
        else:
            self._verbose_print(state, f"[critical_Review] 待审核维度数量：{len(dimension_results)}")
        self._verbose_print(state, f"[critical_Review] 学生报告上下文长度：{len(merged_report_context or '')}")
        self._verbose_print(state, "[critical_Review] 学生报告真实性检查：已开启")
        self._verbose_print(state, "[critical_Review] ============================================================")
        ...
        for dimension_key in all_keys:
            if dimension_key not in dimension_results:
                ...
                self._verbose_print(state, f"[critical_Review] 开始审核维度：{dimension_name} ({dimension_key})")
                self._verbose_print(state, "[critical_Review]   结果完整性检查：未通过")
                self._verbose_print(state, f"[critical_Review]     问题：缺少 {dimension_name} 维度评分结果。")
                audit = {
                    "dimension_key": dimension_key,
                    "dimension_name": dimension_name,
                    "passed": False,
        ...
        self._verbose_print(state, f"[critical_Review] 本轮总体审核结果：{'通过' if passed else '未通过'}")
        self._verbose_print(state, f"[critical_Review] 本轮失败维度：{failed_dimension_keys if failed_dimension_keys else '无'}")
        self._verbose_print(state, "[critical_Review] ============================================================")
...
def _audit_node_factory(agent: IndependentAuditAgent):
    def audit_node(state: ReviewWorkflowState) -> ReviewWorkflowState:
        ...
        current_round = int(state.get("review_round", 0))
        agent._verbose_print(state, f"[critical_Review] 开始第 {current_round} 轮审核")
        if current_round <= 0:
...
def _rescore_node_factory(agent: IndependentAuditAgent):
    def rescore_node(state: ReviewWorkflowState) -> ReviewWorkflowState:
        state = dict(state or {})
        failed_keys = list(state.get("failed_dimension_keys", []))
        agent._verbose_print(state, f"[critical_Review] 审核未通过，要求重评维度：{failed_keys}")
        current_round = int(state.get("review_round", 0))
        ...
        if not isinstance(new_dimension_results, dict) or not new_dimension_results:
            agent._verbose_print(state, "[critical_Review] 重评函数没有返回新的 dimension_results，进入保守输出")
...
def _final_passed_node_factory(agent: IndependentAuditAgent):
    def final_passed_node(state: ReviewWorkflowState) -> ReviewWorkflowState:
        agent._verbose_print(state, "[critical_Review] 审核全部通过")
...
def _final_conservative_node_factory(agent: IndependentAuditAgent):
    def final_conservative_node(state: ReviewWorkflowState) -> ReviewWorkflowState:
        if not state.get("failed_dimension_keys"):
            agent._verbose_print(state, "[critical_Review] 审核未通过但没有可重评维度，进入保守输出")
...
def _final_max_rounds_node_factory(agent: IndependentAuditAgent):
    def final_max_rounds_node(state: ReviewWorkflowState) -> ReviewWorkflowState:
        agent._verbose_print(state, f"[critical_Review] 达到最大自动重评轮数 {state.get('max_review_rounds', agent.max_review_rounds)}，进入保守输出")
